...
var (
	TypeHashMiMC     = []byte("mimc7")
	HashFunctionMiMC HashMiMC
)

type HashMiMC struct{}

func (f HashMiMC) Type() []byte {
	return TypeHashMiMC
}

// Len returns the length of the Hash output
func (f HashMiMC) Len() int {
	return mimc.BlockSize
}

// Hash function expects a list of numbers bytes in little endian
func (f HashMiMC) Hash(b ...[]byte) ([]byte, error) {
	hasher := mimc.NewMiMC()
	for i := 0; i < len(b); i++ {
		var elemX fr.Element
		// swap each element to big endian which is expected by gnark mimc
		elemX.SetBytes(arbo.SwapEndianness(b[i]))
		bElemX := elemX.Bytes()
		if _, err := hasher.Write(bElemX[:]); err != nil {
			return nil, err
		}
	}
	// swap the output to little endian
	return arbo.SwapEndianness(hasher.Sum(nil)), nil
}
